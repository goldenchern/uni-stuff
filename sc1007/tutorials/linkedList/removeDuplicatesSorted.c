...
void removeDuplicatesSortedLL(LinkedList *ll)
{
	...
	int prev = temp->item; 		// prev contains first item
	temp = temp->next;			// temp pointing to second node

	// i tracks iteration count
	// j tracks next node to remove
	for (int i = 1; temp != NULL; ) {
		if (temp->item == prev) {
			temp = temp->next;
			removeNode(ll, i);
		} else {
