...
int moveMaxToFront(ListNode **ptrHead)
{
	...
	// Assume max is first item
	max = (*ptrHead)->item;

	pre = *ptrHead; // Initialise temp pointers
	cur = pre->next;

	// Traverse list to search for max item
	while (cur != NULL) {
		if (cur->item > max) {
			// Set current item to max
			max = cur->item;
			preMax = pre;
		}
	...
	// First item is max, no action required
	if (preMax == NULL) {
		return 0;
	}
