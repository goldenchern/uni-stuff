...
{
	int size = ll->size;
	int splitIndex = (size / 2) + (size % 2);
	ListNode *temp = ll->head;
	
	// Check if linked list is empty
	if (temp == NULL) return;

	resultBackList->head = findNode(ll, splitIndex);
	resultBackList->size = size - splitIndex;
	
	findNode(ll, splitIndex - 1)->next = NULL;
	resultFrontList->head = ll->head;
	resultFrontList->size = splitIndex;
	ll->head = NULL;

	// // Alternative (less efficient) implementation:
	// // Construct list for front half
	// for (int index1 = 0; index1 < splitIndex; index1++) {
	// 	insertNode(resultFrontList, index1, temp->item);
	// 	temp = temp->next;
	// }

	// // Construct list for back half
	// for (int index2 = 0; index2 < size - splitIndex; index2++) {
	// 	insertNode(resultBackList, index2, temp->item);
	// 	temp = temp->next;
	// }
}
