...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{	
	int size1 = ll1->size, size2 = ll2->size;
	int mergedLength = min(size1 + size2, size1 * 2);

	// Traverse first linked list, adding items from the second
	for (int i1 = 1, i2 = 0; i2 < size2 && i1 < mergedLength; i1 += 2, i2++) {
		insertNode(ll1, i1, ll2->head->item);
		removeNode(ll2, 0);
	}
}
...
// Helper function to determine the smaller of two numbers
int min(int a, int b) {
	return (a < b) ? a : b;
}
