...
int Connected (Graph g)
{
    ...
    // Initial node for DFS
    // can be set to any VALID vertex as this is an undirected graph
    const int INITIAL = 1;
    int temp, unvisitedNeighbours;
    
    // Push vertex to stack and mark visited
    push(&s, INITIAL);
    g.visited[INITIAL-1] = 1;
    ...
    while (!isEmptyStack(s)) { 
        ...
        // g.matrix and g.visited: V is from 0 to |V|-1
        for (int i = 0; i < g.V; i++) {
            // Check if adjacent and unvisited
            if (g.matrix[temp-1][i] && !g.visited[i]) {
